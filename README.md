...
- **Auto-refresh**
  Live data with automatic refresh every 60 seconds without losing context.

- **Multi-user Access**
  Session-based authentication using each user’s own OpenShift credentials.

- **In-cluster SSO**
  When deployed on OpenShift with the included `deployment.yaml`, users authenticated in the OpenShift console are automatically logged into OVTools via oauth-proxy. No token required. Compatible with standalone OpenShift and Hypershift.
...
When deployed in-cluster with `deploy/openshift/deployment.yaml`, authentication is handled automatically by `ose-oauth-proxy`. Users already logged into the OpenShift console access OVTools without any extra step. Per-user RBAC is enforced through Kubernetes user impersonation.
