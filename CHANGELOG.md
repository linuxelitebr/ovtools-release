...
### Added
- A health check names the virtual machines holding a duplicate MAC address, with the namespaces and networks they are on. The cluster raises its own alert when duplicates appear, but says only that they exist. Two machines sharing a network are already having their traffic confused for each other and are reported as an error; two on separate networks hold a duplicate that has not met itself yet and are reported as a warning.
