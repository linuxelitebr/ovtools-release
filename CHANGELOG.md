...
### Added
- Topology: drilling into a storage class, network, or NAD that holds a large number of items now groups them by namespace first, so you can open the busy namespace instead of drawing every item at once. It replaces the "show anyway" prompt on those views.

### Changed
- Topology: the overview now colours each node by its real CPU and memory usage (the higher of the two) instead of by how many VMs it holds, matching the Nodes tab. A node with no metrics is shown in neutral grey, and hovering a node shows its CPU and memory figures.
