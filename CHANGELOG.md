...
### Fixed
- Fleet: when a VM reports no hostname — from the guest agent or its own configuration — the details show its internal cluster DNS name (the Internal FQDN, the same value the OpenShift console shows) instead of a blank.

## [0.7.1] 2026-07-12

### Added
- Fleet: the VM details show a Source — the OpenShift template a VM was created from, or "Migrated from VMware" for VMs imported by the Migration Toolkit for Virtualization.

### Fixed
- Fleet: the Eviction strategy shows the effective value — a VM that sets none inherits the cluster-wide default — instead of appearing blank.
- Fleet: the hostname falls back to the VM's configured hostname when the guest agent does not report one.
- Fleet: the Activity header no longer overlaps the feed text as it scrolls.
- Topology: the Network view asks before drawing a very large network (hundreds of VMs on a shared network) instead of rendering an unreadable graph, matching the Storage and Namespace views. The loading indicator now stays visible while a large graph renders.

## [0.7.0] 2026-07-12

### Added
- Fleet: the selected VM now shows live dials for CPU, memory, storage, network and disk I/O, plus an availability dial for how much of the last 30 days it was running.
- Fleet: the VM details list its network interfaces (which NIC is on which network, with its IP and MAC address), its storage volumes, and its snapshots, and show how the vCPUs are arranged into sockets and cores.
- Fleet: the tree and activity panes can be resized and collapsed, and the tab reopens on the VM you last looked at.
- Topology: double-clicking an object — a node, VM, PVC, storage class, network or NAD — opens its tab filtered to that item.

### Changed
- Topology draws each object as an icon (a rack server for a node, stacked disks for a storage class, a disk for a PVC, a monitor for a VM, a folder for a namespace, and network glyphs) instead of plain shapes.
- Fleet VM details are grouped into a Hardware and a Configuration column.

### Fixed
- Topology: clicking a VM in the Network view now shows its IP addresses, and the Storage and Namespace views show the VM's disks, which were blank.
- Searching the Network, Nodes and Storage-class tabs now matches the network, NAD, node and storage-class names, so opening one from the topology lands on the right row.
...
- After an upgrade the interface loads the new files instead of a stale cached copy.
