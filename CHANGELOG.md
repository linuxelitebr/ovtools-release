...
### Fixed
- Signing in with an "@" in the username (an email-style OpenShift user) now works — the dashboard is no longer empty for those accounts. OpenShift's proxy shortens such names to the part before the "@", and OVTools now recovers the full username so the user's own permissions apply. This needs one extra read permission on the service account, so reapply the OpenShift deployment manifest for it to take effect.
