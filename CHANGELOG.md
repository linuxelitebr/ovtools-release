...
### Added
- The Health tab now covers much more of what can actually stop a virtual machine from running. New checks flag: VMs that are running but not ready, or whose disk dropped out while running; VMs stuck because they can't be scheduled, can't pull their image, or are crash-looping; disk imports that failed and storage claims that were lost or never bound; nodes under memory, disk or process pressure, with an unavailable network, or a read-only filesystem; node network configuration that failed to apply; the OpenShift Virtualization, KubeVirt, CDI, SSP and cluster-networking operators when they are degraded or unavailable; VM pods that failed or were killed; storage that couldn't attach to its node; VMs pointing at a network that no longer exists; live migrations that failed; and the per-node KubeVirt agent when it isn't ready on every node. Every new check appears in the resource-type filter so you can isolate it, and all of it was tuned against a live cluster to avoid false alarms.
- Metric columns are now sortable. The VMs and Nodes tabs sort by their live CPU, memory and network columns, the Disks tab sorts by Read and Write IOPS, and the Datastores tab sorts by type.
- Guardrails and extra detail on the Topology tab. Opening a very large "Storage → PVC → VM" or "Namespace → PVC → VM" view now asks you to confirm first, so a cluster with thousands of claims doesn't freeze the browser, and a one-time note explains what each view shows. Popups now list a VM's volumes in the Storage view and its network attachments in the NAD view, and crowded views fold the overflow into a single "Other" count.

### Fixed
- Automatic refresh used to wipe everything you had selected — search text, filters, sorting and the page you were on. It now keeps all of them.
- The "Snapshots By Age" card on the dashboard was always empty. It now shows the data.
- Health stopped crying wolf. It no longer flags a VM for having no eviction strategy, CPU model or resource limits when those are inherited from the cluster or namespace defaults, which was firing on nearly every VM.

### Improved
- The Topology view dropdown was reordered so the densest view (Network → VMs) comes last.
