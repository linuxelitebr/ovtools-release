...
### Added
- New Fleet tab: browse your VMs grouped by namespace or by node, similar to a vSphere inventory. Selecting a VM shows its details next to a feed of recent activity for it — live migrations, snapshots, and when it was created and last started — with a link to open its console.
- Per-VM availability: the share of the recent window (up to 30 days) that a VM was running, shown in the VM details, in the Fleet tab, and in the exported spreadsheet. It reads from Prometheus, so coverage depends on how long your cluster keeps metrics.

### Changed
- The Storage and Namespace topology views used to draw every PVC at once, which was slow and hard to read on large clusters. They now open as an overview — one node per storage class or namespace, with its count — that you click to drill into.
- Updated the bundled spreadsheet-export and Kubernetes client libraries to their latest releases.
