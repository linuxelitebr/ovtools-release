...
- A read role, `ovtools-reader`, now ships with the deployment manifest. It covers exactly what OVTools reads on a signed-in user's behalf, so granting it is the answer when someone is told a tab is not permitted. It replaces the older advice to grant `cluster-reader`, which does not cover the snapshot resources.
